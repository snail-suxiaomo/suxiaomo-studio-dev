<template>
  ...
  <ActiveRuleBar function-key="05-总表" />
    <p class="hint">基于 00-拆分（全章）、项目配置.md、04-策略、03-诊断，编排章→集映射总表，并回写「总集数」到项目配置.md（让 {config} 真正闭环）。</p>
    ...
    <div class="bar">
      ...
      <button :disabled="!projectId || running" @click="callStage('generate')">生成总表</button>
    ...
    <div v-if="result" class="result">
      ...
      <template v-if="result.ok">
        <div class="checks">
          ...
          <div class="check" v-if="result.total_episodes">
            总集数：{{ result.total_episodes }}（已回写项目配置.md）
          </div>
          <div class="check" v-else>总集数：未解析到</div>
        </div>
...
async function callStage(stage) {
  ...
  result.value = null
  try {
    const d = await api('/novel_summary_table/' + stage, 'POST', { project_id: projectId.value })
    result.value = d
  } catch (e) {
